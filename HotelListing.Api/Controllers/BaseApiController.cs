...
namespace HotelListing.Api.Controllers
{
    ...
    {
        //Helper methods to use in the Controller to convert the Result<T> and Result to ActionResult<T> and ActionResult respectively, and to map errors to appropriate HTTP responses.
        protected ActionResult<T> ToActionResult<T>(Result<T> result)
            => result.IsSuccess ? Ok(result.Value) : MapErrorsToResponse(result.Errors);

        protected ActionResult ToActionResult(Result result)
            => result.IsSuccess ? NoContent() : MapErrorsToResponse(result.Error);
